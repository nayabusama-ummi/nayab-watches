...
async function seedCollections(): Promise<Record<string, string>> {
  ...
  console.log(`[seed-catalog] Upserted ${Object.keys(map).length} collections: ${Object.keys(map).join(', ')}`);
  return map;
}
...
async function seedProducts(collectionsMap: Record<string, string>) {
  const products = [
    ...
    },
    {
      ...
      price: BigInt(2850000), // PKR 28,500,000
      currency: 'PKR',
  ...
  for (const p of products) {
    ...
    console.log(`[seed-catalog] Upserted product: ${createdProduct.name} (${createdProduct.reference})`);
  }
}

async function main() {
  console.log('[seed-catalog] Seeding NAYAB fine watchmaking catalog (collections & products only)...');
  const collectionsMap = await seedCollections();
  await seedProducts(collectionsMap);
  console.log('[seed-catalog] Catalog seed completed successfully without user accounts.');
}

main()
  .catch((e) => {
    console.error('[seed-catalog] Catalog seed failed:', e);
    process.exit(1);
  })
  .finally(async () => {
